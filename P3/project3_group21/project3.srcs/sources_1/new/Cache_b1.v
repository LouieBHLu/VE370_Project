...
module cache_b1(
   ...
   reg [134:0] block [0:3];
   //size of block = 1(valid) + 1(LRU) + 5(tag) + 32 * 4(data) = 134 bits
   ...
   initial begin
      for (i = 0; i < 4; i = i + 1)
         block[i][134] = 1'b0; //set valid bit to be 0
   ...
   always @(read_write, address, write_data) begin
      ...
      hit_miss = 1'b0;
      if (address[4] == 1'b0) begin // set index is 0
         if ((block[0][134] == 1'b1) && (block[0][132:128] == address[9:5])) begin  //the corresponding block is 0
               block_number = 2'b00;
               hit_miss = 1'b1;
         ...
         else if ((block[1][134] == 1'b1) && (block[1][132:128] == address[9:5])) begin //the corresponding block is 1
               block_number = 2'b01;
               hit_miss = 1'b1;
         ...
         if (hit_miss == 1'b0) begin
               if ((block[0][134] == 1'b0) || (block[0][133] == 1'b1)) begin //block 0 is not valid or LRU
                  block_number = 2'b00;
                  block[0][134] = 1'b1; //set valid bit to be 1
                  //set block 0 to be the MRU block and block 1 to be the LRU block
                  block[0][133] = 1'b0; 
                  block[1][133] = 1'b1;
               end
                  ...
                  block_number = 2'b01;
                  block[1][134] = 1'b1; //set valid bit to be 1
                  //set block 1 to be the MRU block and block 0 to be the LRU block
                  block[0][133] = 1'b1; 
                  block[1][133] = 1'b0;
               end
               ...
               if ((block[2][134] == 1'b1) && (block[2][132:128] == address[9:5])) begin  //the corresponding block is 2
               block_number = 2'b10;
               hit_miss = 1'b1;
         ...
         else if ((block[3][134] == 1'b1) && (block[3][132:128] == address[9:5])) begin //the corresponding block is 3
               block_number = 2'b11;
               hit_miss = 1'b1;
         ...
         //relocate the block if there is a miss
         if (hit_miss == 1'b0) begin
               if ((block[2][134] == 1'b0) || (block[2][133] == 1'b1)) begin //block 2 is not valid or LRU
                  block_number = 2'b10;
                  block[2][134] = 1'b1; //set valid bit to be 1
                  //set block 2 to be the MRU block and block 3 to be the LRU block
                  block[2][133] = 1'b0; 
                  block[3][133] = 1'b1;
               end
                  ...
                  block_number = 2'b11;
                  block[3][134] = 1'b1; //set valid bit to be 1
                  //set block 3 to be the MRU block and block 2 to be the LRU block
                  block[2][133] = 1'b1; 
                  block[3][133] = 1'b0;
               end
   ...
   // // Load data from main memory when there is a miss
   // if (hit_miss == 1'b0) begin
   ...
   //       #1 
   //       block[block_number][127:0] = memory_read_data;
   //       block[block_number][132:128] = address[9:5];
   // end
   // // Write data through to main memory when there is a hit
   // else begin
   //       #1 
